        ...
        <aside class="w-64 bg-white text-gray-800 flex flex-col justify-between hidden md:flex z-20 shadow-xl border-r">
            <div>
                ...
                <nav class="p-4 space-y-2 mt-4">
                    ...
                    <button onclick="switchTab('categories')" id="nav-categories" class="w-full flex items-center gap-3 px-4 py-3 text-slate-400 hover:bg-slate-800 hover:text-white rounded-xl font-medium transition-colors">
                        <i class="fa-solid fa-tags w-5"></i> Danh mục món ăn
                    </button>
                </nav>
            </div>
                        ...
                        <div id="tab-categories" class="hidden">
                            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-6">
                                <h3 class="font-bold text-gray-800 mb-4 flex items-center gap-2">
                                    <i class="fa-solid fa-circle-plus text-orange-500"></i> Thêm danh mục mới
                                </h3>
                                <form method="post" action="${pageContext.request.contextPath}/admin/dashboard" class="flex flex-wrap gap-3 items-end">
                                    <input type="hidden" name="action" value="ADD_CATEGORY">
                                    <div class="flex-1 min-w-40">
                                        <label class="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-wide">Quán ăn</label>
                                        <select name="merchantId" required class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-200 focus:border-orange-400 bg-white">
                                            <option value="">-- Chọn quán --</option>
                                            <c:forEach var="m" items="${listMerchants}">
                            ...
                            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                                <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                                    <h3 class="font-bold text-gray-800 flex items-center gap-2">
                                        <i class="fa-solid fa-tags text-orange-500"></i> Danh sách danh mục
                                        <span class="bg-orange-100 text-orange-600 text-xs font-black px-2.5 py-0.5 rounded-full">${listCategories.size()}</span>
                                    </h3>
                                </div>
                                <div class="overflow-auto">
                                    <table class="w-full text-sm">
                                        <thead class="bg-gray-50 text-gray-500 uppercase text-xs tracking-wider">
                                            <tr>
                                                <th class="px-6 py-3 text-left font-bold">Quán ăn</th>
                                                <th class="px-6 py-3 text-left font-bold">Danh mục</th>
                                                <th class="px-4 py-3 text-center font-bold">Thứ tự</th>
                                                <th class="px-4 py-3 text-center font-bold">Trạng thái</th>
                                                <th class="px-4 py-3 text-center font-bold">Hành động</th>
                                            </tr>
                                        </thead>
                                        <tbody class="divide-y divide-gray-100">
                                            <c:forEach var="cat" items="${listCategories}">
                                                <tr class="hover:bg-gray-50 transition-colors">
                                                    <td class="px-6 py-3 font-medium text-gray-700">${cat.shopName}</td>
                                                    <td class="px-6 py-3 font-bold text-gray-900">${cat.name}</td>
                                                    <td class="px-4 py-3 text-center text-gray-500">${cat.sortOrder}</td>
                                                    <td class="px-4 py-3 text-center">
                                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${cat.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}">
                                                            <span class="w-1.5 h-1.5 rounded-full ${cat.active ? 'bg-green-500' : 'bg-gray-400'}"></span>
                                                            ${cat.active ? 'Hiện' : 'Ẩn'}
                                                        </span>
                                                    </td>
                                                    <td class="px-4 py-3 text-center">
