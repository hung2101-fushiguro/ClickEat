/* =========================================================
   ...
   One-shot script: Create DB -> Drop old tables -> Create schema -> Trigger -> Seed
   Fixed:
...
-- 1. Thêm cột tên xe
ALTER TABLE dbo.ShipperProfiles
ADD vehicle_name NVARCHAR(100) NULL;

-- 2. Thêm cột biển số xe
ALTER TABLE dbo.ShipperProfiles
ADD license_plate NVARCHAR(20) NULL;
GO

-- (Tùy chọn) Cập nhật dữ liệu mẫu cho các shipper hiện có
UPDATE dbo.ShipperProfiles SET vehicle_name = N'Honda Vision', license_plate = N'29A1-12345' WHERE user_id = (SELECT id FROM dbo.Users WHERE phone=N'0900000007');
UPDATE dbo.ShipperProfiles SET vehicle_name = N'Yamaha Exciter', license_plate = N'59B2-67890' WHERE user_id = (SELECT id FROM dbo.Users WHERE phone=N'0900000008');
GO
/* =========================================================
   16) SEED DATA (no GO inside this block)
   ========================================================= */
