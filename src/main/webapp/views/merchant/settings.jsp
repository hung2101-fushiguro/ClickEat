...
<html lang="vi">
    ...
    <head>
        ...
        <style>
            ...
            .tog-thumb            { position:absolute; top:2px; left:2px; width:1.25rem; height:1.25rem;
            background:white; border-radius:50%; box-shadow:0 1px 2px rgba(0,0,0,.2);
            border: 1px solid #d1d5db;
            transition:transform .2s ease; }
            .tog input:checked ~ .tog-track { background:#22c55e; } /* green-500 */
    ...
    <body class="bg-gray-50 min-h-screen flex">
                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm">
                        ...
                        <div id="tab-security" class="tab-content hidden space-y-6">
                            <div>
                                ...
                                <div class="relative">
                                    <input type="password" id="currentPw" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-gray-50 outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all" placeholder="••••••••"/>
                                    <button onclick="togglePw('currentPw', this)" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="password" id="newPw" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-gray-50 outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all" placeholder="••••••••"/>
                                    <button onclick="togglePw('newPw', this)" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                                        <span class="material-symbols-outlined text-[20px]">visibility_off</span>
                                    </button>
                                </div>
                            </div>
                            <div>
                                ...
                                <div class="relative">
                                    <input type="password" id="confirmPw" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-gray-50 outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all" placeholder="••••••••"/>
                                    <button onclick="togglePw('confirmPw', this)" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                                        <span class="material-symbols-outlined text-[20px]">visibility_off</span>
                                    </button>
                                </div>
                            </div>
                            <div id="pwError" class="hidden px-4 py-3 bg-red-50 border border-red-200 rounded-xl text-sm text-red-600"></div>
                            <div class="flex justify-end pt-4">
                                <button onclick="changePw()" class="bg-primary text-white px-6 py-3 rounded-lg font-semibold hover:bg-orange-600 shadow-md">Đổi mật khẩu</button>
                            </div>
                        </div>
        ...
        <script>
            ...
            function fillAll(from, to) {
                hoursData.forEach(d => { d.open=true; d.from=from; d.to=to; });
                renderHours();
            }
            function saveHours() {
            ...
            function changePw() {
                ...
                if(!cur||!nw||!conf){ err.textContent='Vui lòng điền đầy đủ mật khẩu.'; err.classList.remove('hidden'); return; }
                if(nw!==conf){ err.textContent='Mật khẩu mới không khớp.'; err.classList.remove('hidden'); return; }
                if(nw.length<6){ err.textContent='Mật khẩu mới phải có ít nhất 6 ký tự.'; err.classList.remove('hidden'); return; }
            ...
            function showToast(msg) {
                ...
                if(!t){ t=document.createElement('div'); t.id='toast'; t.className='fixed bottom-20 md:bottom-6 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-sm font-medium px-5 py-3 rounded-xl shadow-lg z-[100] transition-all duration-300 opacity-0'; document.body.appendChild(t); }
