...
<html lang="vi" class="h-full">
    <head>
        ...
        <style>
            .timeline-line { position: absolute; left: 1.2rem; top: 2.5rem; bottom: 0.5rem; width: 2px; background: #e5e7eb; }
        </style>
    </head>
        ...
        <div class="flex h-full">
            ...
            <div class="p-4 md:p-8 max-w-7xl mx-auto">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <!-- ═══════════════════════════════════════════
                    LEFT: Receipt card + action buttons
                    ═══════════════════════════════════════════ -->
                    <div class="space-y-4">
                        ...
                        <div class="bg-white rounded-2xl shadow-xl border border-gray-100" style="border-top: 8px solid #c86601;">
                            ...
                            <!-- Card header: code, time, status badge -->
                            <div class="p-6 text-center border-b border-dashed border-gray-200">
                                ...
                                isDelivered ? 'bg-gray-100 text-gray-600' :
                                ...
                                <c:choose>
                                    <c:when test="${isNew}">🔵 Đơn mới</c:when>
                                        <c:when test="${st == 'MERCHANT_ACCEPTED'}">🟡 Đã nhận</c:when>
                                            ...
                                            <!-- ═══════════════════════════════════════════
                                            RIGHT: Info panels (2 cols in lg)
                                            ═══════════════════════════════════════════ -->
                                            <div class="lg:col-span-2 space-y-6">

                                                <!-- Delivery info: customer + shipper -->
                                                ...
                                                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                                                    ...
                                                    <div class="relative">
                                                        ...
                                                        <div class="space-y-7">
                                                            ...
                                                            <c:choose>
                                                                ...
                                                                <c:otherwise>
                                                                    ...
                                                                    <!-- Step 4 (top): Giao hàng thành công -->
                                                                    <div class="flex items-start gap-4 ${isDelivered ? '' : 'opacity-40'}">
                                                                        <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 z-10 relative
                                                                        ${isDelivered ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-400'}">
                                                                ...
                                                                <!-- Step 3: Sẵn sàng lấy hàng -->
                                                                <div class="flex items-start gap-4 ${step3Done || step3Current ? '' : 'opacity-40'}">
                                                                    <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 z-10 relative
                                                                    ${step3Done ? 'bg-green-500 text-white' : step3Current ? 'bg-primary text-white' : 'bg-gray-200 text-gray-400'}">
                                                                    <c:choose>
                                                            ...
                                                            <!-- Step 2: Đang chuẩn bị món -->
                                                            <div class="flex items-start gap-4 ${step2Done || step2Current ? '' : 'opacity-40'}">
                                                                <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 z-10 relative
                                                                ${step2Done ? 'bg-green-500 text-white' : step2Current ? 'bg-primary text-white' : 'bg-gray-200 text-gray-400'}">
                                                                <c:choose>
                                                        ...
                                                        <!-- Step 1 (bottom): Đặt hàng thành công -->
                                                        <div class="flex items-start gap-4">
                                                            <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 z-10 relative bg-green-500 text-white">
                                                                <span class="material-symbols-outlined text-[18px]">check</span>
                                                            </div>
                        ...
                        <form id="prepFormDetail" method="POST" action="${pageContext.request.contextPath}/merchant/orders/detail">
                            ...
                            <div class="p-6">
                                <div class="grid grid-cols-3 gap-3 mb-6">
                                    <button type="button" onclick="selectPrepDetail(10)"
                                        class="prep-btn-detail py-3 rounded-xl border-2 border-gray-200 text-gray-700 font-semibold text-sm hover:border-primary hover:text-primary transition-all">
                                        10 phút
                                    </button>
                                    <button type="button" onclick="selectPrepDetail(15)"
                                        class="prep-btn-detail py-3 rounded-xl border-2 border-gray-200 text-gray-700 font-semibold text-sm hover:border-primary hover:text-primary transition-all">
                                        15 phút
                                    </button>
                                    <button type="button" onclick="selectPrepDetail(20)"
                                        class="prep-btn-detail py-3 rounded-xl border-2 border-primary text-primary bg-orange-50 font-semibold text-sm">
                                        20 phút
                                    </button>
                                    <button type="button" onclick="selectPrepDetail(30)"
                                        class="prep-btn-detail py-3 rounded-xl border-2 border-gray-200 text-gray-700 font-semibold text-sm hover:border-primary hover:text-primary transition-all">
                                        30 phút
                                    </button>
                                    <button type="button" onclick="selectPrepDetail(45)"
                                        class="prep-btn-detail py-3 rounded-xl border-2 border-gray-200 text-gray-700 font-semibold text-sm hover:border-primary hover:text-primary transition-all">
                                        45 phút
                                    </button>
                                    <button type="button" onclick="selectPrepDetail(60)"
                                        class="prep-btn-detail py-3 rounded-xl border-2 border-gray-200 text-gray-700 font-semibold text-sm hover:border-primary hover:text-primary transition-all">
                                        60 phút
                                    </button>
                                </div>
                                <div class="flex gap-3">
                                    <button type="button" onclick="closePrepModalDetail()"
                                        class="flex-1 py-2.5 border border-gray-200 rounded-xl text-gray-600 font-semibold text-sm hover:bg-gray-50 transition-all">
                                        Hủy
                                    </button>
                                    <button type="submit"
                                        class="flex-1 py-2.5 bg-primary text-white rounded-xl font-bold text-sm hover:bg-primary-dark transition-all shadow-lg shadow-primary/20">
                                        Xác nhận nhận đơn
                                    </button>
                                </div>
                            </div>
                ...
                <div id="cancelModalDetail" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/60 backdrop-blur-sm">
                    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
                        ...
                        <form id="cancelFormDetail" method="POST" action="${pageContext.request.contextPath}/merchant/orders/detail">
                            ...
                            <div class="p-6 space-y-3">
                                <button type="button" onclick="selectReasonDetail('Hết nguyên liệu')"
                                    class="cancel-reason-btn-detail w-full text-left px-4 py-3 rounded-xl border-2 border-gray-200 text-gray-700 font-medium text-sm hover:border-red-300 hover:bg-red-50 hover:text-red-700 transition-all">
                                    🧂 Hết nguyên liệu
                                </button>
                                <button type="button" onclick="selectReasonDetail('Bếp quá bận')"
                                    class="cancel-reason-btn-detail w-full text-left px-4 py-3 rounded-xl border-2 border-gray-200 text-gray-700 font-medium text-sm hover:border-red-300 hover:bg-red-50 hover:text-red-700 transition-all">
                                    🔥 Bếp quá bận
                                </button>
                                <button type="button" onclick="selectReasonDetail('Cửa hàng đóng sớm')"
                                    class="cancel-reason-btn-detail w-full text-left px-4 py-3 rounded-xl border-2 border-gray-200 text-gray-700 font-medium text-sm hover:border-red-300 hover:bg-red-50 hover:text-red-700 transition-all">
                                    🏪 Cửa hàng đóng sớm
                                </button>
                                <button type="button" onclick="selectReasonDetail('Ngoài khu vực giao hàng')"
                                    class="cancel-reason-btn-detail w-full text-left px-4 py-3 rounded-xl border-2 border-gray-200 text-gray-700 font-medium text-sm hover:border-red-300 hover:bg-red-50 hover:text-red-700 transition-all">
                                    📍 Ngoài khu vực giao hàng
                                </button>
                                <button type="button" onclick="closeCancelModalDetail()"
                                    class="w-full py-2.5 border border-gray-200 rounded-xl text-gray-500 font-semibold text-sm hover:bg-gray-50 transition-all mt-2">
                                    Hủy bỏ
                                </button>
                            </div>
                        </form>
                ...
                <script>
                    ...
                    document.getElementById('prepModalDetail').addEventListener('click', function(e) {
                        if (e.target === this) closePrepModalDetail();
                    });
                    ...
                    document.getElementById('cancelModalDetail').addEventListener('click', function(e) {
                        if (e.target === this) closeCancelModalDetail();
                    });
                </script>
            </body>
        </html>
