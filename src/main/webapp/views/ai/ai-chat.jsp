            ...
            <div class="flex gap-6 flex-1 min-h-0">
                <div class="flex-1 bg-white rounded-[2rem] shadow-sm border border-gray-100 flex flex-col overflow-hidden">
                    ...
                    <div class="p-4 bg-white shrink-0">
                        ...
                        <form method="POST" action="${pageContext.request.contextPath}/ai" onsubmit="(function(f){var i=f.querySelector('#msgInput');var m=(i&&i.value||'').trim();if(m){var l=document.getElementById('messagesList');if(l){var d=document.createElement('div');d.className='flex gap-4 justify-end';d.innerHTML='<div><div class=&quot;bg-primary text-white p-4 rounded-2xl rounded-tr-sm text-[15px] font-medium leading-relaxed max-w-xl shadow-md shadow-orange-200&quot;></div></div>';d.querySelector('div div').textContent=m;l.appendChild(d);}}})();showLoading();" class="flex items-center gap-2 bg-gray-50 p-1.5 rounded-full border border-gray-200 focus-within:border-primary/50 focus-within:ring-2 focus-within:ring-primary/10 transition-all">
